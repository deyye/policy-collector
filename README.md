...
| 省级 | 31 | 每省 1 个政策栏目；其中 **29 个省级地区已有实采数据** |
| 演示 | 1（停用） | 本地合成样例，不参与官网采集 |

**未接通的 2 个省级地区**：湖北（`fgw.hubei.gov.cn`）、甘肃（整个 `gansu.gov.cn` 域）
——整站 HTTP 412，**连无头 Chrome 也过不去**（同一命令在新疆/海南/江西约 10 秒返回，
在这两家卡住 6 分钟以上仍未返回）。不是"没找对入口"，建议判死并写进交付说明。

> ⚠️ **口径提醒**：`SELECT COUNT(DISTINCT region)` 会得到 31，但那是
> **29 个省级地区 + 国家 + 样例**。对外只能说"**29/31 省级地区**"，
> 不能拿 31 当"覆盖 31 个省"。
...
| 全来源独立验收 | **34/34 通过，0 需关注**（含归属检查） | `docs/source-audit-all.json`；隔离临时库跑，语料为各栏目**抽样**，不等于全省覆盖率 |
| 全来源实际采集一轮 | 34 个来源跑完，**5 分 45 秒**；库内 412 → **607 条**，覆盖 **29/31 省级地区**（缺湖北、甘肃） | 规则模式、每来源限 10 份，见下表"口径陷阱"；单源失败隔离不中断整批 |
...
| 汇报后近期 | **湖北、甘肃两家整域 WAF 未接通**（无头 Chrome 亦过不去） | 要么上浏览器指纹层，要么判死并写进交付说明；当前建议后者 |
...
31 省里**已接通 29 个**（另加国家，共 **30 个地区 / 34 个启用来源**）。
本轮新增 **新疆、海南、江西**——三家原先都记在"最贵的一档"，实测**两家根本不需要、一家换个入口就行**：
...
> 海南与江西又是**"把可用材料当成缺失"**的同族错误（本项目已第五次）：
> 一次是文章型判据太窄、一次是正文容器表没有 `div.article1`。
> **判据一窄，站点就从"没政策"变成"有政策"。**
