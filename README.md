...
### 接入结果：启用来源 11 → 26，覆盖省份 9 → 21

新增 16 个省级栏目，经 `scripts/audit_sources.py`（列表发现 → 正文 → 附件 → 入库）逐个验收，
**15 个通过**。验收证据见 `docs/province-source-audit.json`。
...
**验收脚手架的已知盲区**：`audit_sources.py` 只能验证"列表能发现、正文能解析、附件能入库"，
**验证不了"收进来的文件是否真属于该地区"**。福建那个聚合页就是这样漏过去的
（15 条全是国家发改委转载，单看机制完全正常）。
这条只能靠人工抽查列表前若干条的**实际来源目录**来判断——
批量跑一遍全部来源、逐个核对列表指向，比单源验收更能暴露这类问题。
