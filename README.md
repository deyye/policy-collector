...
网页“模型配置”可完成全套操作：从 **11 家预设**（DeepSeek、通义千问、Kimi、智谱、硅基流动、
MiniMax、火山方舟、OpenAI、本机 vLLM、本机 Ollama、其他兼容接口）里选一家，
地址/模型名/密钥变量名自动带出；填密钥保存后检查连接，再用**一键启用/停用**控制是否使用大模型。
停用后采集一律走本地规则，且来源页不再提供“自动处理”选项——开关与采集口径是同一件事，
不允许出现“页面显示停用、采集仍在调模型”。也可命令行交互输入密钥：
...
`LLMClient` 对 `localhost`/`127.0.0.1` 放行免密，因此本机 vLLM / Ollama 可直接接入；
远程服务仍要求非空密钥，不会自动下载或部署模型。

**配置优先级**（2026-09-15 调整）：shell 显式环境变量 > 本机 `data/llm.local.json`（界面保存）> 根目录 `.env` > YAML。

`.env` 之所以降到界面配置之后：两者都是"本机文件"，而界面是更明确的一次操作。
调整前 `.env` 压着界面配置，用户在页面上改密钥**永远不生效**——他填、他保存、页面显示"已保存"，
实际用的还是 `.env` 那份。shell 里 `export` 的变量用于 CI/运维，仍最高优先，不受影响。
页面上会**如实标出**当前密钥来自哪一层，来源不一致时不必猜。

密钥不回显，本机文件不加密且不入Git；演示或分享数据目录时不要带出密钥。用 `doctor` 检查配置，`llm-check` 才会发出最小模型请求并产生少量用量。
