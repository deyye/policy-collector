...
### 未接通省份的排查结论（2026-09-13，避免重复侦察）

31 省里已接通 24 个。剩下 7 个的实测障碍如下——**都做了只读探测，结论可直接作为下次起点**：

| 省 | 障碍 | 实测细节 |
|---|---|---|
| 湖北 | HTTP 412 | 首页即被 WAF 拦截，需浏览器指纹层 |
| 甘肃 | HTTP 412 | 同上 |
| 内蒙古 | 代理 502 | 首页经环境代理返回 502；栏目页可以取到（`fgw.nmg.gov.cn` 的通知公告已接通） |
| 天津 | 列表 JS 渲染 | `/xxfb/tzggx/` 60KB 但零文章链接；无 `recordset`/`dataproxy`/`unitbuild` 标记 |
| 辽宁 | 列表 JS 渲染 | 用**慧点 CMS**（`/uiFramework/huilan-jquery-ui`、`/fgw/template/fgw.js`）；页面有 `isOpenStaticPageList` 标识，值得顺着查其静态列表开关 |
| 山东 | 需找对入口 | 首页只链到 `/col/col91475/`（**跳转桩**，指向 `/col/col91477/`「调查研究」栏目，非政策）；真正的政策栏目入口未在首页导航文本中出现 |
