...
### 未接通省份的排查结论（更新至 2026-09-14，避免重复侦察）

31 省里已接通 27 个。9/13 那份清单里的天津、辽宁、山东**本轮已接通**
（前两个是"链接不在 href 里"，山东是"栏目在政府信息公开下"）。剩下 4 个的实测障碍如下——
**都做了只读探测，结论可直接作为下次起点**：

| 省 | 障碍 | 实测细节 |
|---|---|---|
| 湖北 | HTTP 412 | 首页即被 WAF 拦截，需浏览器指纹层 |
| 甘肃 | HTTP 412 | 同上 |
| 海南 | 列表走 AJAX | `/sfgw/0500/zcwj.shtml` 等页面均为壳（36–53 个链接、0 篇文章型）；
  `/common/search/<hash>` 直接 GET 返回 28KB 但只有 27 个链接、同样无列表，
  内容由 POST 获取 → 需逐个逆向其查询接口 |
| 江西 | 入口待找 | 首页无文章链接；猜测的 `/col/col14951/` 返回 404 |
| 新疆 | 入口待找 | 首页只链到 `common_list.shtml` 类栏目页，需看锚文本继续定位 |

内蒙古虽首页经代理返回 502，但其通知公告栏目（`fgw.nmg.gov.cn`）**已接通**，不计入未覆盖。

**这一档的共同点**：不是"站点不可用"，而是**列表数据要经 JS/接口拿**。
按仓库既有的接入成本排序，这是最贵的一档——每站接口都不同，需逐站逆向。
9/13 把它们全归为这一档是**过度概括**：其中三家实测根本不需要，只要把
"链接可能在 onclick 里""文章形态表可能不全"这两件事补上就解决了。
