...
31 省里**已接通 28 个**（另加国家，共 29 个地区 / 33 个启用来源）。
本轮新增 **新疆、海南**——两家原先都记在"最贵的一档"，实测都**不需要**：
...
> 海南这一条是**本项目第四次**"把可用材料当成缺失"的同类错误（前三次：`partial` 附件被算缺失、
> 正文容器未覆盖就判"整页仅供复核"、`attachments_unparsed` 按状态而非有无正文计数）。
...
**真正剩下的 2 个**（都是硬骨头，且性质不同）：

| 省 | 障碍 | 本轮实测细节 |
|---|---|---|
| 湖北 | HTTP 412 | 首页即被 WAF 拦截（`fgw.hubei.gov.cn`），需浏览器指纹层 |
| 甘肃 | HTTP 412 | **整个 `gansu.gov.cn` 域**都拦（`fzgg`、`www` 均 412，http/https 同） |
| 江西 | 入口在 CMS 接口后 | Chrome 渲染**可行**：首页静态 0 条锚文本命中 → 渲染后 19 条，详情形态 `/jxsfzhggwyh/col/col<N>/content/content_<19位>.html`。但**能访问的 `col14586` 是国家发改委转载栏目**（15 条里 11 条中央文件），启用即重演福建问题；`政务公开 col47604` 是空容器（页面自报 `childCount:0, articleCount:0`），正文由 CMS 的 JSON 接口给 → **需逐站逆向接口，暂不启用** |

内蒙古虽首页经代理返回 502，但其通知公告栏目（`fgw.nmg.gov.cn`）**已接通**，不计入未覆盖。

**教训沉淀**：这一档里，"看似要 JS"与"真的拿不到"是两件事。
三次误判各有其因——链接在 onclick 里、文章形态表不全、文章型判据太窄；
真正需要 JS/接口的，往往不是列表页本身，而是**列表所在的那个容器是空壳**（江西即是）。
