...
class RuleClassifier:
    ...
    def classify(self, doc: Document) -> Classification:
        text = _norm((doc.title or "") + " " + (doc.content or ""))
        head = _norm((doc.title or "") + " " + (doc.content or "")[:400])
        ...
        cls = Classification(
            ...
            doc_type=doc_type,
            need_review=need_review or is_inv == "pending",
...
边界注意：规定项目能耗准入条件→准入类；安排能耗指标保障→保障类。细则类文件多类混杂默认 need_review=true。"""
    ...
    def classify(self, doc: Document) -> Optional[Classification]:
        if not self.available:
            return None
        user = (
            "标题：" + (doc.title or "") + "\n文号：" + (doc.wenhao or "") + "\n发文机关：" + (doc.issuing_authority or "")
            + "\n正文（前3000字）：\n" + (doc.content or "")[:3000]
        )
        data = self.client.chat_json(self.SYSTEM_PROMPT_TMPL + "\n" + self.boundary_hints, user)
        if not data:
            return None
        return self._validate(doc, data)

    def _validate(self, doc: Document, data: dict) -> Classification:
        is_inv = str(data.get("is_investment_policy", "pending"))
        if is_inv not in ("yes", "no", "pending"):
            is_inv = "pending"
        cats = data.get("category", [])
        if isinstance(cats, str):
            cats = [c.strip() for c in cats.split(",") if c.strip()]
        cats = [c for c in cats if c in CATEGORY_CN][:3]
        doc_type = str(data.get("doc_type", "其他"))
        allowed_types = {"正式政策", "申报通知", "解读", "征求意见稿", "项目批复", "其他"}
        if doc_type not in allowed_types:
            doc_type = "其他"
        return Classification(
            is_investment_policy=is_inv,
            category=",".join(cats),
            category_names=",".join(CATEGORY_CN.get(c, c) for c in cats),
            doc_type=doc_type,
            need_review=bool(data.get("need_review", False)) or (is_inv == "pending") or not cats,
            reason=str(data.get("category_reason", ""))[:500],
            evidence=str(data.get("evidence", ""))[:500],
            confidence=float(data.get("confidence", 0)) if isinstance(data.get("confidence"), (int, float)) else None,
            model_version=self.client.cfg.effective_model or "llm",
            reviewer_hint=str(data.get("reviewer_hint", ""))[:200],
        )
...
class Classifier:
    ...
    def classify(self, doc: Document, prefer: str = "llm") -> Classification:
        if prefer == "llm" and self.llm.available:
